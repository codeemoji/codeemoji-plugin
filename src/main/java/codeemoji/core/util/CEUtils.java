...
public enum CEUtils {
    ...
    public static boolean isPluralForm(@Nullable final String name) {
        if (null != name && 1 < name.trim().length()) {
            final var word = identifyLastWordWithUpperCase(name);
    ...
    public static boolean isGenericType(@NotNull final PsiElement element, @Nullable final PsiTypeElement typeElement) {
        final var tpl = searchGenericTypesList(element);
        if (null != tpl) {
    ...
    private static boolean isSuffixWhiteList(@NotNull final String name) {
        ...
        try (final var is = classLoader.getResourceAsStream("suffix_whitelist_plural.json")) {
            if (null != is) {
                ...
                if (null != je) {
                    final var suffixes = je.getAsJsonArray("suffixes");
    ...
    public static boolean isDateDBType(@NotNull final PsiType psiType) {
        if (psiType instanceof final PsiClassType classType) {
    ...
    public static String uncapitalizeAsProperty(final String str) {
        if (CEUtils.notHasLength(str) || (1 < str.length() && Character.isUpperCase(str.charAt(0)) &&
                Character.isUpperCase(str.charAt(1)))) {
            return str;
        }
        return CEUtils.changeFirstCharacterCase(str, false);
    }

    public static boolean notHasLength(@Nullable final String str) {
        return (null == str || str.isEmpty());
    }

    private static String changeFirstCharacterCase(final String str, @SuppressWarnings("SameParameterValue") final boolean capitalize) {
        if (CEUtils.notHasLength(str)) {
            return str;
        }
        final char baseChar = str.charAt(0);
        final char updatedChar;
        if (capitalize) {
