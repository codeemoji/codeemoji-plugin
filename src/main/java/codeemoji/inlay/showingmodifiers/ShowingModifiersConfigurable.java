...
public record ShowingModifiersConfigurable(ShowingModifiersSettings settings) implements ImmediateConfigurable {
    ...
    private @NotNull JPanel prepareClassPanel(@NotNull ChangeListener changeListener) {
        ...
        var publicClass = new JCheckBox(PUBLIC_SYMBOL_EMOJI + PUBLIC, settings().query(PUBLIC_CLASS));
        var defaultClass = new JCheckBox(DEFAULT_SYMBOL_EMOJI + DEFAULT, settings().query(DEFAULT_CLASS));
        var finalClass = new JCheckBox(FINAL_SYMBOL_EMOJI + FINAL, settings().query(FINAL_CLASS));
        var abstractClass = new JCheckBox(ABSTRACT_SYMBOL_EMOJI + ABSTRACT, settings().query(ABSTRACT_CLASS));
    ...
    private @NotNull JPanel prepareFieldPanel(@NotNull ChangeListener changeListener) {
        ...
        var publicField = new JCheckBox(PUBLIC_SYMBOL_EMOJI + PUBLIC, settings().query(PUBLIC_FIELD));
        var defaultField = new JCheckBox(DEFAULT_SYMBOL_EMOJI + DEFAULT, settings().query(DEFAULT_FIELD));
        var finalField = new JCheckBox(FINAL_VAR_SYMBOL_EMOJI + FINAL, settings().query(FINAL_FIELD));
        var protectedField = new JCheckBox(PROTECTED_SYMBOL_EMOJI + PROTECTED, settings().query(PROTECTED_FIELD));
        var privateField = new JCheckBox(PRIVATE_SYMBOL_EMOJI + PRIVATE, settings().query(PRIVATE_FIELD));
        var staticField = new JCheckBox(STATIC_SYMBOL_EMOJI + STATIC, settings().query(STATIC_FIELD));
        var volatileField = new JCheckBox(VOLATILE_SYMBOL_EMOJI + VOLATILE, settings().query(VOLATILE_FIELD));
        var transientField = new JCheckBox(TRANSIENT_SYMBOL_EMOJI + TRANSIENT, settings().query(TRANSIENT_FIELD));
    ...
    private @NotNull JPanel prepareMethodPanel(@NotNull ChangeListener changeListener) {
        ...
        var publicMethod = new JCheckBox(PUBLIC_SYMBOL_EMOJI + PUBLIC, settings().query(PUBLIC_METHOD));
        var defaultMethod = new JCheckBox(DEFAULT_SYMBOL_EMOJI + DEFAULT, settings().query(DEFAULT_METHOD));
        var finalMethod = new JCheckBox(FINAL_SYMBOL_EMOJI + FINAL, settings().query(FINAL_METHOD));
        var protectedMethod = new JCheckBox(PROTECTED_SYMBOL_EMOJI + PROTECTED, settings().query(PROTECTED_METHOD));
        var privateMethod = new JCheckBox(PRIVATE_SYMBOL_EMOJI + PRIVATE, settings().query(PRIVATE_METHOD));
        var staticMethod = new JCheckBox(STATIC_SYMBOL_EMOJI + STATIC, settings().query(STATIC_METHOD));
        var abstractMethod = new JCheckBox(ABSTRACT_SYMBOL_EMOJI + ABSTRACT, settings().query(ABSTRACT_METHOD));
        var synchronizedMethod = new JCheckBox(SYNCHRONIZED_SYMBOL_EMOJI + SYNCHRONIZED, settings().query(SYNCHRONIZED_METHOD));
        var nativeMethod = new JCheckBox(NATIVE_SYMBOL_EMOJI + NATIVE, settings().query(NATIVE_METHOD));
        var defaultInterfaceMethod = new JCheckBox(DEFAULT_INTERFACE_SYMBOL_EMOJI + DEFAULT + " (in interfaces)", settings().query(DEFAULT_INTERFACE_METHOD));
    ...
    private void addChangeListener(@NotNull JCheckBox checkBox, @NotNull ShowingModifiers.Modifier modifier, @NotNull ChangeListener changeListener) {
        checkBox.addChangeListener(event -> {
            settings().update(modifier, checkBox.isSelected());
